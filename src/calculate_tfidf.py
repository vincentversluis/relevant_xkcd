...
def get_tfidf(df: pd.DataFrame, doc_id_col: str, text_col: str, n_gram_length: int=1) -> pd.DataFrame:
    # Preprocess
    ...
    title_text_tfidf_df = tfidf_df.melt(id_vars=doc_id_col, var_name='tokens', value_name='tfidf')
    title_text_tfidf_df = title_text_tfidf_df[title_text_tfidf_df['tfidf'] > 0]
    
    # Rename columns
    title_text_tfidf_df = title_text_tfidf_df.rename(columns={'tokens': 'token'})
...
# %% ANALYSE DATA
for n_gram_length in tqdm(range(1, n_gram_max_length+1), desc='Calculating tf-idf scores...'):
    # Get tfidf for titles
    tfidf_title_df = get_tfidf(xkcd_properties_df, 'xkcd_id', 'title', n_gram_length=n_gram_length)
    tfidf_title_df['heading'] = 'title'

    # Get tfidf for title texts
    tfidf_title_text_df = get_tfidf(xkcd_properties_df, 'xkcd_id', 'title_text', n_gram_length=n_gram_length)
    ...
    tfidf_transcripts_df = get_tfidf(xkcd_transcripts_df, 'xkcd_id', 'text', n_gram_length=n_gram_length)
    ...
    tfidf_rest_of_explanations_df = get_tfidf(xkcd_rest_of_explanations_df, 'xkcd_id', 'text', n_gram_length=n_gram_length)
